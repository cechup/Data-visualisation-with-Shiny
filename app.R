...
artists$Country <- gsub("UK", "GB", artists$Country)
artists$Country <- gsub("SU", "RU", artists$Country)
    ...
    fluidRow(
      ...
      column(4, offset = 4,
             tags$div(id = ns("legend"),
                      style = "border:1px solid black; padding: 5px; background-color: #f9f9f9; font-size: 90%;",
                      tags$h4("Color legend", style="margin-top: 0; margin-bottom: 5px;"),
                      tags$ul(style = "list-style-type: none; padding-left: 0; margin-top: 0;",
                              tags$li(tags$span(style = "color: #FF6666; font-size: 150%;", "\u25CF"), " AFRICA", style="margin-bottom: 2px;"),  # Reduced margin between items
                              tags$li(tags$span(style = "color: #6699FF; font-size: 150%;", "\u25CF"), " ASIA", style="margin-bottom: 2px;"),
                              tags$li(tags$span(style = "color: #66FF99; font-size: 150%;", "\u25CF"), " EUROPE", style="margin-bottom: 2px;"),
                              tags$li(tags$span(style = "color: #FF9900; font-size: 150%;", "\u25CF"), " LATIN AMERICA", style="margin-bottom: 2px;"),
                              tags$li(tags$span(style = "color: #CC99FF; font-size: 150%;", "\u25CF"), " NORTH AMERICA", style="margin-bottom: 2px;"),
                              tags$li(tags$span(style = "color: #FFFF66; font-size: 150%;", "\u25CF"), " OCEANIA")
                      )
             )
      )    
      ),
...
plot_barChart <- function (cleaned_df, firstYear, secondYear) {
  ...
  total_count = sum(grouped_df$n)
  grouped_df$relative_freq = grouped_df$n / total_count
  grouped_df$`Main Genre` <- fct_relevel(grouped_df$`Main Genre`, "Others", after = Inf)
  ggplot(grouped_df, aes(x=`Main Genre`, y=relative_freq)) + 
    labs(x = "Genres", y = "Percentage of songs published") +
